...
module.exports = (grunt) => {
  ...
  grunt.initConfig({
    ...
    concat: {
      license: {
        ...
        files: {
          'dist/block-accordion-plugin/plugin.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ],
          'dist/block-accordion-plugin/plugin.min.js': [
            'src/text/license-header.js',
            scratchPluginMinPath
          ]
        }
    ...
    copy: {
      css: {
        files: [
          { src: [ 'CHANGELOG.txt', 'LICENSE.txt' ], dest: 'dist/block-accordion-plugin', expand: true }
        ]
      }
    },
  ...
  grunt.registerTask('version', 'Creates a version file', () => {
    grunt.file.write('dist/block-accordion-plugin/version.txt', BUILD_VERSION);
  });
